...
import { extractErrorDetails, formatHash, formatNumber } from "@/utils";
...
const StakeAction = ({ usdBalance }: { usdBalance: usdBalanceProps }) => {
  const { stakeAmount, setStakeAmount, isStaking, setIsStaking } = useStakeStore();
  const {
    data: hash,
    ...
    <>
      {!showModal && (
        <Button
          ...
          // onClick={() => {
          //   posthog.capture('STAKE_EVENT', { property: stakeAmount })
          //   handleStake();
          // }}
          // disabled={
          //   !parseFloat(stakeAmount) ||
          //   Number(stakeAmount) > Number(usdBalance.currentBalance) ||
          //   isStaking.loading
          // }
          is_loading={isStaking.loading}
          variant="primary"
        >
          Stake
        </Button>
                ...
                {isStaking.status !== "idle" &&
                  isStaking.status !== "pending" && (
                    <ModalHeader
                      ...
                      supportingText={
                        isStaking.status === "success"
                          ? "Your WELF has been successfully deposited into the staking pool."
                          : error
                          ? extractErrorDetails(error.cause as string)
                              .detailedMessage
                          : "Your WELF could not be deposited into the staking pool. Please try again or contact support if the issue persists."
                      }
                    />
                  )}
                      ...
                      <div className="py-2x px-4x bg-weak rounded-md flex justify-between items-center w-full">
                        ...
                        <div className="text-body-s-medium text-foreground-weakest ">
                          <span className="text-foreground-weak mr-1">
                            {formatNumber(Number(stakeAmount))}
                          </span>
                          WELF
                        </div>
                      </div>
