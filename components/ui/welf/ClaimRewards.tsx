...
const ClaimRewards = ({userStakes}: {userStakes: UserStakes[]}) => {
 
  return (
    <Accordion
      type="multiple"
      className="flex flex-col gap-6"
      defaultValue={userStakes ? userStakes?.[0]?.rewards?.periods?.map(
        (_, ind) => `item-${ind}`
      ) : []}
    >
            ...
            <AccordionContent className="mt-3x pb-0">
              <div className="border border-default rounded-lg overflow-hidden bg-weak">
                ...
                <div className="p-4 flex flex-col gap-4">
                  {stake.rewards?.periods.map((item, index) => (
                    <div
                    ...
                    >
                      ...
                      <div className="flex justify-end items-center gap-0.5">
                        <Image
                          ...
                          width={16}
                        />
                        {item.reward.toFixed(item.reward < 1 ? 5 : 2)}
                      </div>
                    </div>
                  ))}
                </div>
